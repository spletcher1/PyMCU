...
class TSL2591:
    ...
    def ValidateTimeAndGain(self):
        isOkay=True
        tmp_time = self.theSensor.integration_time
        tmp_gain = self.theSensor.gain

        if(tmp_time!= self.theSensor._integration_time):
            self.theSensor.integration_time = self.theSensor._integration_time  
            isOkay=False                      

        if(tmp_gain!= self.theSensor._gain):
            self.theSensor.gain = self.theSensor._gain           
            isOkay=False
    ...
    def IncreaseSensitivity(self):       
        ...
        if self.theSensor.integration_time == adafruit_tsl2591.INTEGRATIONTIME_100MS :
            self.theSensor.integration_time = adafruit_tsl2591.INTEGRATIONTIME_300MS
        elif self.theSensor.integration_time == adafruit_tsl2591.INTEGRATIONTIME_300MS :
            self.theSensor.integration_time = adafruit_tsl2591.INTEGRATIONTIME_500MS
        elif self.theSensor.gain == adafruit_tsl2591.GAIN_LOW :
            self.theSensor.gain = adafruit_tsl2591.GAIN_MED
    ...
    def DecreaseSensitivity(self):        
        ...
        if self.theSensor.gain == adafruit_tsl2591.GAIN_HIGH :
            self.theSensor.gain = adafruit_tsl2591.GAIN_MED
        elif self.theSensor.gain == adafruit_tsl2591.GAIN_MED :
            self.theSensor.gain = adafruit_tsl2591.GAIN_LOW
        elif self.theSensor.integration_time == adafruit_tsl2591.INTEGRATIONTIME_500MS :
            self.theSensor.integration_time = adafruit_tsl2591.INTEGRATIONTIME_300MS
        elif self.theSensor.integration_time == adafruit_tsl2591.INTEGRATIONTIME_300MS :
            self.theSensor.integration_time = adafruit_tsl2591.INTEGRATIONTIME_100MS
        else:
    ...
    def GetLUX(self):
        if self.theSensor is None:
            return -1
        try:
            if (self.ValidateTimeAndGain()==False):
                return -99
